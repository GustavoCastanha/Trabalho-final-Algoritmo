...
def cadastrar_clientes():
    ...
    for verificacao in clientes.values():  #"Verificacao", cada cliente cadastrado em "clientes" ele entra e verifica se ja existe o email
...
def compras(cpf):
    ...
    while True:
        ...
        try:
            ...
            produto = produtos[id_produto - 1]  # -1 pois o indice da lista começa em 0 até 19
...
def mostrar_carrinho(cpf):  
    ...
    if ver_itens.lower() == 's': #lower pega a caracter e deixa em minusculo
        ...
        for i, item in enumerate(cliente['carrinho'], 1): #emumerate cria um "lista" com cada produto mantendo a ordem 1-2-3-4..., o 1 no final indica que começa do 1
...
def pagar_conta(cpf):
    ...
    if cliente['desconto'] == 0:
        ...
        jogar = input("Quer testar sua sorte para ganhar desconto? (s/n):").lower()   
        
        while jogar not in ['s', 'n']:
            jogar = input("Entrada inválida! Quer testar sua sorte para ganhar desconto? (s/n):").lower()
        
        if jogar == 's':
            teste_sua_sorte(cpf) #chama a função do desconto
...
def teste_sua_sorte(cpf):
    ...
    try:
        ...
        while palpite < 0 or palpite > 100:
            ...
            palpite = int(input("Digite seu palpite (0-100): "))
